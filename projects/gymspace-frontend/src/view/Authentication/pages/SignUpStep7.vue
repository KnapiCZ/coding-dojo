...
// step 6 a step 7 udelat komponetu, ktera bude mit jako props typ toho, co se ma nastavovat a pak se bude jen renderovat jinak podle toho, co se nastavuje
const router = useIonRouter()
const userStore = userDataStore()
const currentNumber = ref(85)
...
const updateTransformations = () => {
  ...
  visibleScaleSteps.value.forEach((item) => {
    ...
    if (element) {
      ...
      const scaleFactor = 1 / Math.pow(1.5, distanceFromCenter) // Apply scaling: 1 / (2^distance)
      const opacityFactor = Math.max(0.2, 1 - distanceFromCenter * 0.15) // Decrease opacity with distance
...
const setWeigth = () => {
  userStore.addToDict("registerData", "weight", currentNumber.value)
  router.push("step-8")
}

// Update transformations on mount to initialize correctly
onMounted(() => {
  updateTransformations()
})
</script>
...
<template>
  <IonPage>
    <IonContent class="sign-up-content top-padding-notch" :scroll-y="false">
      <img src="../../../assets/images/weight.png" class="sign-up-background-image" alt="Cake">
      <h1 class="sign-up">
        Choose your weight...
      </h1>
      ...
      <IonButton fill="solid" class="sign-up-button" @click="setWeigth">
        NEXT
      </IonButton>
    </IonContent>
  </IonPage>
